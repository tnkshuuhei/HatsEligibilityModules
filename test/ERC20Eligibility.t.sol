...
contract ERC20EligibilityTest is Deploy, Test {
  ...
  function setUp() external {
    ...
    bytes memory otherImmutableArgs = abi.encodePacked(address(mintableERC20), MIN_BALANCE);

    //create ERC20HatsEligbilityModule instance
    instance = ERC20Eligibility(factory.createHatsModule(address(implementation), 0, otherImmutableArgs, ""));
  }
}
...
contract SetUp is ERC20EligibilityTest {
  function test_Immutables() external {
    assertEq(instance.ERC20_TOKEN_ADDRESS(), address(mintableERC20), "incorrect token address");
